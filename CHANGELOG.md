...
- Bundle a `flags` webfont (`public/flags.woff2`) and use it wherever a country
  ...
  grid), so flags render on platforms/browsers whose OS lacks flag-emoji glyphs
  (e.g. Windows/Chromium). Scoped to those spots only, with the platform emoji
  fonts as fallback for everything else
